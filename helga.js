...
tampilTanggal = tanggal + " " + bulan1 + " " + tahun
...
➡️ Silahkan Chat Owner untuk Sewa Bot.
...
`
sendButLocation(from, `${menu}`, "*Made With By Itsuki Botz*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
╰────
`
sendButLocation(from, `${menu}`, `${uwu}Big Thanks To\n\nAllah SWT\nHelga Zexs${uwu}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}about`,buttonText:{displayText:'ABOUT🌱'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
break
case 'menulain':
 if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
	teks =
...
│ • ${prefix}bugreport [ query ]
╰────`
gam = await getBuffer(pporang)
but = [
          { buttonId: `${prefix}allmenu`, buttonText: { displayText: '<<BACK TO MENU' }, type: 1 },
          { buttonId: `${prefix}about`, buttonText: { displayText: 'RULES' }, type: 1 },
          { buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER' }, type: 1 }
        ]
        sendButImage(from, teks, "©Farhan Ganz", gam, but)
break
case 'groupmenu':
 if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
	teks =
...
case 'menu':
        ...
        if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
        ptod = "6282138919347@s.whatsapp.net"
        stit = `${NomorOwner}@s.whatsapp.net`
        stod = `${sender}`
        uwu = '```'
       stst = await helga.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
...
Thanks To:
1.Helga Zexs (Base Ori)
`
sendButLocation(from, `${menu}`, `${uwu}WARNING(PERINGATAN)\nBot sedang dalam tahap Pengembangan, Filtur eror atau menu tidak muncul mohon maklumi, tunggu info selanjutnya!${uwu}`, {jpegThumbnail:ofrply}, [{buttonId:`allmenu`,buttonText:{displayText:'ALL MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'OWNER'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
break
case 'allmenu':
...
stod = `${sender}`
 ...
 footerText: '*_© Itsuki Botzゑ_*',
 description: `Hai @${stod.split('@')[0]}\nSaya Itsuki Bot\nFiltur eror atau menu tidak muncul mohon maklumi\nKarena Masih proses pengembangan`,
 sections: [
                     {
 ...
 rows: [
                          {
                              "title": "ownermenu",
                              "description": 'FILTUR KHUSUS OWNER BOT',
                              "rowId": ""
                            
                           },
                           {
                              "title": "groupmenu",
                              "description": 'FILTUR KHUSUS DI GRUP',
                              "rowId": ""
                           },
                           {
                              "title": "downloadmenu",
                           ...
                           {
                              "title": "stickermenu",
                              "description": 'FILTUR STICKER',
                              "rowId": ""
                           },
                           {
                           "title": "animemenu",
                              "description": 'FILTUR ANIME',
                              "rowId": ""
                           },
                           {
                           "title": "photooxymenu",
                              "description": 'FILTUR PHOTO OXY',
                              "rowId": ""
                           },
                           {
                           "title": "menulain",
                              "description": 'UNTUK MELIHAT MENU LAINNYA',
                              "rowId": ""
                           },
                           {
                              "title": "about",
                                    "description": 'RULES PENGGUNA BOT',
                              "rowId": ""
                           }         
...
case 'loli':
       ...
       if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
        ...
        //── 「 Owner Menu 」 ──//          
              ...
              case 'setmenu':
if (!isOwner && !mek.key.fromMe) return reply('Lu bukan Owner GOBLOOG')
if(args[0] == 'ori'){
menusimpel = true
reply('Sucsess')
...
break
case 'asmaulhusna':
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/asmaulhusna?apikey=$cb18b3f1027cedbd8b5abb1a`)
...
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/photooxy1/love?apikey=cb18b3f1027cedbd8b5abb1a&&text=${ini_txt}`).then((gambar) => {
helga.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Done Subscribe Herman Chanel`, quoted : ftrol})
})
break
case 'shadow':
if (args.length == 0) return reply(`Teksnya Mana ?\nContoh : ${prefix + command} herman Chanel`)
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/photooxy1/shadow?apikey=cb18b3f1027cedbd8b5abb1a&&text=${ini_txt}`).then((gambar) => {
helga.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Done Subscribe Herman Chanel`, quoted : ftrol})
})
break
case 'cup':
if (args.length == 0) return reply(`Teksnya Mana ?\nContoh : ${prefix + command} herman Chanel`)
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/photooxy1/cup?apikey=cb18b3f1027cedbd8b5abb1a&&text=${ini_txt}`).then((gambar) => {
helga.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Done Subscribe Herman Chanel`, quoted : ftrol})
})
break
case 'romance':
if (args.length == 0) return reply(`Teksnya Mana ?\nContoh : ${prefix + command} herman Chanel`)
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/photooxy1/romance?apikey=cb18b3f1027cedbd8b5abb1a&&text=${ini_txt}`).then((gambar) => {
helga.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Done Subscribe Herman Chanel`, quoted : ftrol})
})
break
case 'smoke':
if (args.length == 0) return reply(`Teksnya Mana ?\nContoh : ${prefix + command} herman Chanel`)
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/photooxy1/smoke?apikey=cb18b3f1027cedbd8b5abb1a&&text=${ini_txt}`).then((gambar) => {
helga.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Done Subscribe Herman Chanel`, quoted : ftrol})
})
break
case 'nature3d':
if (args.length == 0) return reply(`Teksnya Mana ?\nContoh : ${prefix + command} herman Chanel`)
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/photooxy1/nature3d?apikey=cb18b3f1027cedbd8b5abb1a&&text=${ini_txt}`).then((gambar) => {
helga.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Done Subscribe Herman Chanel`, quoted : ftrol})
})
break
case 'carvedwood':
if (args.length == 0) return reply(`Teksnya Mana ?\nContoh : ${prefix + command} herman Chanel`)
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/photooxy1/carvedwood?apikey=cb18b3f1027cedbd8b5abb1a&&text=${ini_txt}`).then((gambar) => {
helga.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Done Subscribe Herman Chanel`, quoted : ftrol})
})
break
			case 'runtime':
...
case 'benarkah':
  if (!isGroup) return reply(`GRUP ONLY`)
              ...
              helga.sendMessage(from, '*Pertanyaan :* '+benarkah+'\n*Jawaban :* '+ hoh, text, { quoted: ftrol })
              break
       case 'kapankah':
         if (!isGroup) return reply(`GRUP ONLY`)
...
break
case 'bucin':
              helga.sendMessage(from, "Aku memilih untuk sendiri, bukan karena menunggu yang sempurna tetapi butuh yang tak pernah menyerah", text)
              break
...
Giliran = @${tty.player1.split('@')[0]}`
 helga.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     helga.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Command anda salah coba Cek lagi*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer ZukaChan",body:"",previewType:"Helga Base",thumbnail:fs.readFileSync('./helga.jpg'),sourceUrl:"https://wa.me/6282138919347"}}})
	}
    }
    }
    }
